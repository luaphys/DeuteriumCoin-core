...
The RPM spec file provided here is for Deuteriumcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://deuteriumcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Deuteriumcoin git repository](https://github.com/deuteriumcoin/deuteriumcoin) but are not part of
...
Currently only `Source10` is used, for the example `deuteriumcoin.conf` file.
...
in the deuteriumcoin git repository. At present only an SVG version of the Deuteriumcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the deuteriumcoin
...
needed when 0.12.1 is released, a proper fix is already in the Deuteriumcoin git
master branch.
...
idea to build Deuteriumcoin Core against a static version of that library compiled
...
The `deuteriumcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `deuteriumcoin-server` package. At this point, most RPM based
...
The `deuteriumcoin-server` package should have SELinux support. How to properly do
...
Deuteriumcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/deuteriumcoin/). He has been
packaging Deuteriumcoin for Fedora at least since 2012.
...
4. I renamed the `deuteriumcoin` package that contains the Qt GUI to `deuteriumcoin-core` as
that appears to be how the general population refers to it, in contrast to
`deuteriumcoin-xt` or `deuteriumcoin-classic`. I wanted to make sure the general population
