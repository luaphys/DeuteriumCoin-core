...
###   This script attempts to download the signature file SHA256SUMS.asc from deuteriumcoin.org
...
WORKINGDIR="/tmp/deuteriumcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://deuteriumcoin.org/bin/"
VERSIONPREFIX="deuteriumcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'deuteriumcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Deuteriumcoin Core binary release signing key installed?"
   fi
