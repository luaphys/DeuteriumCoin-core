    ...
    nCurrentFilter2(FILTER_ALL)
{
    ...
    proposalsBtn = new DeuteriumcoinPushButton(tr("Fund Proposals"));
    paymentRequestsBtn = new DeuteriumcoinPushButton(tr("Payment Requests"));
    consultationsBtn = new DeuteriumcoinPushButton(tr("Consultations"));
    deploymentsBtn = new DeuteriumcoinPushButton(tr("Deployments"));
    consensusBtn = new DeuteriumcoinPushButton(tr("Consensus Parameters"));
...
void DaoPage::initialize(CProposalMap proposalMap, CPaymentRequestMap paymentRequestMap, CConsultationMap consultationMap, CConsultationAnswerMap consultationAnswerMap, int unit, bool updateFilterIfEmpty)
{
    ...
    for (auto& it: proposalMap)
    {
        ...
        ProposalEntry p = {
            ...
            DeuteriumcoinUnits::formatWithUnit(unit, proposal.nAmount, false, DeuteriumcoinUnits::separatorAlways),
            DeuteriumcoinUnits::formatWithUnit(unit, proposal.nAmount - proposal.GetAvailable(coins), false, DeuteriumcoinUnits::separatorAlways),
    ...
    for (auto& it: paymentRequestMap)
    {
        ...
        PaymentRequestEntry p = {
            ...
            (proposal.nVersion & CProposal::SUPER_VERSION ? "SUPER PROPOSAL: ": "" ) + QString::fromStdString(proposal.strDZeel).left(150) + (proposal.strDZeel.size() > 150 ? "..." : ""),
            "#6666ff",
            DeuteriumcoinUnits::formatWithUnit(unit, prequest.nAmount, false, DeuteriumcoinUnits::separatorAlways),
...
void DaoPage::onDetails() {
    if (!contextHash.isEmpty())
    {
        ...
        if (nCurrentView == VIEW_PROPOSALS)
            ...
            type = "consultation";
        QString link = "https://www.deuexplorer.com/dao/" + type + "/" + contextHash;
