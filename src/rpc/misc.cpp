...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total deuteriumcoin balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            for(const CTxDestination& addr: addresses)
                a.push_back(CDeuteriumcoinAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"deuteriumcoinaddress\"\n"
            "\nReturn information about the given deuteriumcoin address.\n"
            "\nArguments:\n"
            "1. \"deuteriumcoinaddress\"     (string, required) The deuteriumcoin address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"deuteriumcoinaddress\", (string) The deuteriumcoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"deuteriumcoinaddress\", (string) The deuteriumcoin staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"deuteriumcoinaddress\", (string) The deuteriumcoin spending address part of a cold staking address\n"
            "  \"votingaddress\" : \"deuteriumcoinaddress\", (string) The deuteriumcoin voting address part of a cold staking v2 address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CDeuteriumcoinAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CDeuteriumcoinAddress spendingAddress;
            ...
            CDeuteriumcoinAddress spendingAddress;
            address.GetSpendingAddress(spendingAddress);
            CDeuteriumcoinAddress votingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Deuteriumcoin address and we have full public key:
        CDeuteriumcoinAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are deuteriumcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) deuteriumcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CDeuteriumcoinAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CDeuteriumcoinAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"deuteriumcoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"deuteriumcoinaddress\"  (string, required) The deuteriumcoin address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CDeuteriumcoinAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CDeuteriumcoinAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CDeuteriumcoinAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
bool getAddressesFromParamsForHistory(const UniValue& params, std::vector<std::pair<std::pair<uint160, uint160>, AddressHistoryFilter>> &addresses)
{
    if (params[0].isStr()) {
        ...
        {
            CDeuteriumcoinAddress addressVoting, addressStaking, addressSpending;
        ...
        {
            CDeuteriumcoinAddress addressStaking, addressSpending;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            if (address.IsColdStakingv2Address(Params()))
            {
                CDeuteriumcoinAddress addressSpending, addressVoting, addressStaking;
                uint160 hashBytes, hashBytesSpending, hashBytesVoting, hashBytesStaking;
                int type = 0;
...
UniValue getaddresshistory(const UniValue& params, bool fHelp)
{
    ...
    std::map<CDeuteriumcoinAddress, balStruct> balance;
