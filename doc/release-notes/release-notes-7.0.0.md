# Deuteriumcoin v7.0.0 Release Notes

## xDEU upgrades for Deuteriumcoin Core 7.0.0

<[Pull Request 862](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/862)>

This Pull Request introduces various consensus changes related to xDEU signalled by version bit 33. As we did run out of version bits for updates, this PR also introduces a patch to use the nBits field of the block header to signal for version bits greater than 32.
...
### Improvements in the xDEU inter-node mixing protocol.
...
### Optimizations in the xDEU verification routines.

There have been improvements in the verification functions of the cryptographic proofs attached to xDEU transactions, resulting in huge speed increases. Seeing numbers down to 15ms in my laptop.
...
For example, the amount of an output is never assigned space when the output is private. Before it was an 8 byte field filled with zeroes for private transactions. Similar situation was given for DEU outputs in a transaction with xDEU inputs, where the spendingKey and ephemeralKey were not significant but were still consuming 96 bytes of space in total until this patch is applied.
...
`vData` can be used to hold arbitrary data. The original intent of this is to substitute the use of the transaction's field `strDZeel` to hold metadata or program predicates. As the `strDZeel` fields of different xDEU transactions can't be aggregated into one, we were missing the extended use derived from the utility of `strDZeel` for private transactions.

`tokenId` signals the token a private output refers to. Its default value `0x0` refers to xDEU. From `tokenId` it is calculated the generator `H` used in the verification of the Bulletproof Rangeproofs without breaking the zero-sum-balance property in the xDEU verification protocol.
...
Built on top of the same cryptographic protocol which makes xDEU possible, we are adding the possibility to mint confidential tokens on the Deuteriumcoin blockchain. Transactions of those tokens will veil the details of their sender, receiver and amount transferred.

nAssets can represent arbitrary tokens created by users of the Deuteriumcoin network as well as stablecoins with their value pegged and backed by different underlying assets, while benefiting from the privacy-preserving properties of blsCT.
...
xDEU is used to pay the creation fees.
...
- `minttoken <token_id> <xdeu_address> <amount>`

Mints `amount` of tokens to `xdeu_address`. Minting tokens add to the token's current supply and can be executed until the max. supply of a token is depleted.

xDEU is used to pay the minting fees.

- `mintnft <token_id> <nft_id> <xdeu_address> <metadata>`

Mints 1 NFT to `xdeu_address`. 

xDEU is used to pay the minting fees.

- `sendtoken <token_id> <xdeu_address> <amount>`

Sends `amount` of tokens to `xdeu_address` from current wallet.

xDEU is used to paid the transaction fees.

- `sendnft <token_id> <nft_id> <xdeu_address>`

Sends `nft_id` of tokens to `xdeu_address` from current wallet.
...
<[Pull Request 877](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/877)>

This Pull Request introduces the consensus changed needed for a naming system dubbed dotDEU secured by Deuteriumcoin's blockchain and signalled for activation with version bit 34 (using block header's nonce).

Names are only allowed with the suffix `.deu` and to have a maximum length of 64 characters. Only allowed characters are alphanumeric and `-`, and the first character can't be a `-`.

Valid names: `satoshi.deu` `satoshi-nakamoto.deu` `satoshi21.deu` `21satoshi.deu`
Invalid names: `-satoshi.deu` `satoshi_nakamoto.deu`

*RPC Command*: `registername name`

Examples:

`registername alex.deu`

Registration has a cost set by a consensus parameter (initially `10 DEU`) and lasts 400 days worth of blocks (modifiable through consensus parameters). Names can be renewed by everyone (not only the owner) with the rpc command `renewname name`. The name's expiry date will be set to 400 days worth of blocks after the moment of the renewal, and it will have the same cost as a registration.
...
`updatename alex.deu ip 127.0.0.1`
`updatename alex.deu email alex@deu.community`
`updatename personal.alex.deu email alex.home@deu.community`

Names can be resolved through the RPC command `resolvename name`

## DAO protocol upgrade

<[Pull Request 856](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/856)>
...
<[Pull Request 850](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/850)>

This PR adds a new RPC command to show a transaction input and output keys when combined with txindex.

This helps to greatly improve sync time when using a xDEU light wallet.
...
<[Pull Request 855](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/855)>
...
<[Pull Request 870](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/870)>

## Update coin control legacy code

<[Pull Request 847](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/847)>

## build,boost: update download url

<[Pull Request 851](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/851)>

## fix mutex detection when building bdb on macOS

<[Pull Request 852](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/852)>

This PR fixes a bug to prevent the use of DEU outputs together with recently swapped coins from XDEU to DEU as per an issue reported in Discord by mxaddict.

## Removed program_options dependency from boost

<[Pull Request 858](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/858)>

## Updated the configure.ac python version requirement to match upstream changes

<[Pull Request 859](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/859)>

## Replaced boost::function with std::function calls 

<[Pull Request 860](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/860)>

## Replaced boost::filesystem with fs:: from upstream code changes

<[Pull Request 863](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/863)>

## Remove using namespace std; usage 

<[Pull Request 864](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/864)>

## Removed the use of PAIRTYPE 

<[Pull Request 865](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/865)>

## Updated the build aux pthread file

<[Pull Request 866](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/866)>

## Add asm support in configure.ac and updated crc32c based on upstream changes

<[Pull Request 867](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/867)>

## Update stressor for super dao and combined consensus changes

<[Pull Request 868](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/868)>

## Updated univalue based on upstream changes

<[Pull Request 869](https://github.com/deuteriumcoin/deuteriumcoin-core/pull/869)>
