...
class GetColdStakingAddress(DeuteriumcoinTestFramework):
    ...
    def run_test(self):
        ...
        try:
            ...
            assert("Staking address is not a valid Deuteriumcoin address" in e.error['message'])

        try:
            self.nodes[0].getcoldstakingaddress(address_one, coldstaking_address_one)
            make_addr_fail = False
        except JSONRPCException as e:
            assert("Spending address is not a valid Deuteriumcoin address" in e.error['message'])
